...
| `TWIN` | **what this twin produced in this sandbox** | a `/veris/data` row or `/veris/requests` exchange, with ids **and a saved excerpt** | any mock; a stubbed provider; another sandbox |
...
**The twin is not the vendor** — strong evidence about it, not the same thing.
Never write "I measured" unqualified for something a twin produced.
...
`scripts/ledger.sh init --task <id>` prints the field contract; `check` validates
it; `--against-diff` closes Gate 4. Every measurement ends as one of:
...
redacted excerpt under `snapshots/`. Identifiers, paths, commands and excerpts

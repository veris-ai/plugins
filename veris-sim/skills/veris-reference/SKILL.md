...
description: Knowledge the veris-sim commands read on demand - the twin's control surface, fault rows, the proxy run, sandbox data, webhooks, trust, troubleshooting, the PR evidence shape. Not a command; setup, build and fix name the file to read.
...
| `data.md` | the data a sandbox holds: seeding rows, isolation, reset, promote, snapshots |
| `webhooks.md` | the application receives callbacks |
| `trust.md` | an SDK refuses the proxy's certificate |
| `troubleshooting.md` | what the receipt, the trace, an exit code and a vendor-shaped error each mean |

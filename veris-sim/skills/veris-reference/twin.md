...
| Which operations does this service implement? Does it serve *this* call? | `GET {control_url}/veris/operations` — the whole supported surface in one read, on every service. REST answers `{"method","path"}` and GraphQL `{"type","field"}`, both under `operations`; a service with a hosted MCP surface adds `mcp`, carrying its `path` and the `tools` it resolves. `?surface=rest`, `?surface=graphql` or `?surface=mcp` narrows it — any other value is a `422`, and a surface this service does not serve answers empty. `total` counts what came back. **Paths are templates** (`/repos/{owner}/{repo}/issues`), so match your concrete URL against the pattern; a path or method absent from the list is not served, which also catches a wrong API version (`/v2/...` on a `/v1` service). Two things this list is *not*. It is the union of deterministic handlers and reviewed fallbacks, so being listed means the twin will answer that call, **not** that it implements it deterministically — do not read it as fidelity. And it is not the vendor's `tools/list`, which is served verbatim and so includes tools that fail closed; the catalogue is the only place that distinction shows |

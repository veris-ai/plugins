...
Three things about the trace. It records your own `/veris/*` seeding and read-back
too, as tier `control`, so an unfiltered page after a heavy seed can be mostly your
own writes; filter to the tier you mean before counting anything. Credentials,
cookies, OAuth codes and private keys show as `[REDACTED]`; the query string is not
recorded. And times are the sandbox's own clock, in UTC; a row whose status is a
bare dash is a hang fault, where the twin sent nothing.

...
| Which operations does this service implement? Does it serve *this* call? | `GET {control_url}/veris/operations` — the whole supported surface in one read, on every service; `?surface=` narrows it to `rest`, `graphql` or `mcp`. What the response will not tell you: paths are **templates** (`/repos/{owner}/{repo}/issues`), so match your concrete URL against the pattern, and absent means not served; being listed says the twin will answer that call, **never that it answers faithfully**; and `mcp` holds the tools this service resolves, where the vendor's own `tools/list` is verbatim and includes tools that fail closed |

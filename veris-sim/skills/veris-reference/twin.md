...
| Which operations does this service implement? Does it serve *this* call? | `GET {control_url}/veris/operations`, on every service; `?surface=` narrows to `rest`, `graphql` or `mcp`. Paths are **templates** — match your concrete URL against the pattern. Listed means the twin answers that call, **not that it answers faithfully**. `mcp` holds the tools it resolves; the vendor's `tools/list` is verbatim and includes tools that fail closed |

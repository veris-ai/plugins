...
| Which operations does this service actually implement? | Two doors. `GET {control_url}/veris/operations` answers `{"service","total","operations":[{"method","path"}]}` where a service publishes one — but publishing is opt-in and off by default, so expect a `404` that means only *no catalogue here*. The door that always works: **arm a fault at the operation and read the rejection.** `POST {control_url}/veris/data` with a `faults` row is validated against the same published surface the catalogue would list, so it answers per operation — see below |
| Does the twin serve *this* call? | `POST {control_url}/veris/data` with one `faults` row for that `method` + `path`. Three answers, measured: **`200 {"added":{"faults":1}}`** — served, and you have now armed a real fault, so delete it if you did not want it. *Served* is the union of deterministic handlers and reviewed fallbacks, so a `200` means the twin will answer that call, **not** that it implements it deterministically — do not read it as fidelity. **`422 "<METHOD> is not published for path '<path>'; allowed methods: ['POST']"`** — the path is served, that method is not, and it names the ones that are. **`422 "path '<path>' does not match a published operation of this service"`** — not served at all; this also catches a wrong API version (`/v2/...` on a `/v1` service). A rejected row is validated before anything is written, so a `no` costs nothing. Unlike a vendor-shaped `404`, every one of these is a control-plane answer and settles the question |

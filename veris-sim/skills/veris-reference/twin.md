...
| Which operations does this service actually implement? | `GET {control_url}/veris/operations` — one call, and where a service publishes one it answers `{"service","total","operations":[{"method","path"}]}` and settles coverage outright. Publishing is **opt-in and off by default**, so a `404` means only that this service publishes no catalogue: it falls through to the vendor emulation and comes back in the vendor's error shape. Read nothing else into it — no shipped service publishes one today |

...
A third tier, for a machine with no Docker daemon, is [hosted.md](hosted.md): the
same unmodified code in a hosted box wired to the twin, with the trace as the receipt.
That file points to the supported provider recipe.

...
Retain the relevant command/result or redacted excerpt when it informs the change.
Reuse that evidence in the [short handoff](evidence.md); no per-probe ledger is required.
State a material unverified assumption as such, and resolve a finding that contradicts
